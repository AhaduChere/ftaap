<script setup lang="ts">
    ...
    <template>
      ...
      <section
        class="pt-20 md:pt-10 bg-[#f7feff] w-screen h-auto md:h-screen grid grid-cols-1 md:grid-cols-2 place-items-center md:space-y-10 space-y-20">
        <div class="absolute top-[70px] left-1 flex">
          <select
            id="stuents"
            v-model="selectedStudentId"
            name="students"
            class="bg-transparent border-none border-0 p-0 m-0 shadow-none text-[#2e777e] font-semibold">
          ...
          <div v-if="selectedStudent" class="flex flex-wrap">
            ...
            <button
              type="button"
              class="ml-5 px-2 py-0 justify-self-end bg-[#2e777e] text-white rounded hover:bg-[#276166] transition-colors"
        ...
        <div
          ...
          class="md:w-3/4 w-7/8 h-[18rem] bg-white border border-[#2e777e] border-1 drop-shadow-lg rounded-md col-span-1 md:col-span-2">
        ...
        <div class="col-span-1 md:col-span-2 flex items-center justify-center md:flex-row flex-col md:w-3/4 w-full gap-3">
          ...
          <div v-if="selectedStudent" class="md:w-1/2 w-6/8 h-[18rem] bg-white border border-[#2e777e] border-1 drop-shadow-lg rounded-md">
            ...
            <VocabularyChart class="pt-3 pl-3" :student-score-id="selectedStudent?.id" />
          </div>
