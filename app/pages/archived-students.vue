...
async function unarchiveStudent(s: { id: number; firstName: string; lastName: string }) {
  if (
    !confirm(
      `Restore ${s.firstName} ${s.lastName} to the active student list?`
    )
  ) {
    return
  }
  ...
  try {
    await $fetch(`/api/students/${s.id}/unarchive`, {
      method: 'POST',
    })
    await refresh()
  } catch (e) {
...
<template>
  ...
  <div class="p-6 space-y-6 bg-[#f7feff] min-h-screen">
    <div class="w-full bg-white border border-[#2e777e] drop-shadow-lg rounded-md overflow-visible">
      ...
      <!-- Teal header bar (like manage-students) -->
      <header class="bg-[#2e777e] text-white px-4 py-3 md:px-6 md:py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <div>
          <h1 class="text-lg md:text-xl font-semibold tracking-tight">
            Archived Students
          </h1>
          <p class="text-xs md:text-sm text-white/80">
            View previously archived students and restore them to the active roster when needed.
          </p>
        </div>
        <!-- No Add button here — this page is for restoring only -->
      </header>

      <!-- Filters row (styled like controls on manage-students) -->
      <section
      ...
      <!-- Table body, same card feel as manage-students -->
              ...
              >
                ...
                <td class="py-2 px-2 md:px-3 text-right">
                  <button
                    ...
                    :disabled="unarchiveLoadingId === s.id"
                    @click="unarchiveStudent(s)"
                  >
                    <span v-if="unarchiveLoadingId === s.id">
                      Restoring…
                    </span>
                    <span v-else>
                      Restore
