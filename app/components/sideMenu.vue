...
<template>
  <Transition name="slide">
    <div
      v-if="isOpen"
      class="fixed left-0 top-0 h-screen w-full sm:w-80 bg-[#2e777e] drop-shadow-lg sideMenuDropShadow flex flex-col"
    >
      <!-- Header -->
      <div class="flex items-center justify-between px-4 py-4 border-b border-white/40">
        ...
        <Icon
          name="mdi:close"
          class="text-[#e2fafc] text-3xl cursor-pointer"
          @click="closeMenu"
        />
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        <ul class="py-2">
          ...
          <li>
            <div
            class="px-4 py-3 font-semibold text-lg text-[#e2fafc] border-b border-white/30 hover:bg-[#205a5f] no-select"
            >
            Dashboard
            </div>
